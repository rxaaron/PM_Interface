<?php
    ...
    //Actual generation of tables.
        ...
        $select=$db->query("SELECT A.Rx_ID, A.Patient_Name, A.Patient_Group, A.Rx_Number, A.HOA_Code, A.Sig_QuantityPerDose, B.Drug_ID, B.Drug_Name, C.Line1, C.Line2, C.Line3, A.Pack, D.Patient_ID FROM ".$_SESSION['prefix']."_Rx AS A INNER JOIN Sig AS C ON A.Sig_Code=C.Sig_Code INNER JOIN Drug AS B ON A.Drug_Code = B.Drug_Code INNER JOIN Patient AS D ON A.Patient_Name = D.Patient_Name WHERE A.HOA_ID <> '0' ORDER BY A.Patient_Name, B.Drug_Name;");
    ...
    if($select){
        ...
        while($results=$select->fetch_object()){
            if($pname===$results->Patient_Name){
                ...
                echo "<h3><code>".$results->Patient_Group."</code> ".$pname." <div class=\"pad4 btn-group\"><button class=\"btn btn-mini btn-success\" onclick=\"changeall('".$pname."',true);\">Add All</button><button class=\"btn btn-mini btn-danger\" onclick=\"changeall('".$pname."',false);\">Remove All</button></div><button data-toggle=\"modal\" href=\"#ptadmin\" class=\"btn btn-danger pull-right\" onclick=\"patientadmin(".$results->Patient_ID.");\"><i class=\"icon-trash icon-white\"></i></h3>";
                echo $tablestart;
                echo "<tr><td><input type=\"checkbox\" value=\"".$results->Rx_Number."\" id=\"".$results->Rx_ID."\" onclick=\"checkchange(this.id,this.value);\"";
                if($results->Pack == 1){
                    echo "checked=\"checked\">";
